...
扫描和启动 smoke；真实邮箱投递只作为人工验收，不把任何邮箱地址、授权码或任务注册
信息写入提交、标签或 Release 附件。
...
- 当前阶段只准备和归档源码，不创建 `v1.3.0` 标签、不推送、不发布 GitHub Release，
  也不签名或公证。完成三平台验收并取得明确授权后，才进入“正常公开发布”步骤。
