...
- 中文单位和学科会先转换为学术索引可识别的检索条件；同一位学者在不同求学、工作单位留下的记录，会在 ORCID、共同作品 DOI 等证据足够时合并。
- ORCID 或作者 ID 不会被盲目信任。如果索引记录混入明显无关的别名、单位或学科，程序会改用代表作作为身份锚点，并提示需要核验。
- 同一位学者被数据库拆成多个作者 ID 时，会依据 ORCID、共同 DOI、单位、研究方向、共同作者和专著关系整合为一个档案；搜索结果会显示整合数量和依据。
- 已关注学者的身份记录会在首次升级及之后每七天联网核验一次。发现新的作者 ID 后会自动写回本地数据，无需取消后重新关注。
- 本版不包含邮件、定时任务或提醒设置。
...
- v1.1.1 会在首次成功联网更新时整合同一学者的重复关注项，并保留其中最早的关注日期以及所有文章状态。
