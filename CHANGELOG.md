...
- settings 升级为 version 3，主研究数据仍为 version 7；保留既有 API Key、关注项、
  收藏、已读、忽略、翻译和缓存。v1.3.0 目前仅为待发布候选，不创建标签或 Release。

### 开发与验证状态

- Windows 修复测试包已通过最终包启动、CSS/JavaScript、空白后重试迁移、持久化、导入、
  自动关闭、端口冲突和隐私检查，并由使用者确认本机使用正常。
- 35 项确定性测试、lint 与生产构建均通过；统一 1.3.0 源码已准备交接。
- Windows 正式候选以及 macOS arm64/x64 原生包仍须从同一最终提交重新构建；macOS
  Keychain、LaunchAgent 和真实 SMTP 投递尚未完成正式发布验收。
