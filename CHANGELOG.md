...
- 学者搜索改为顺序无关的证据图聚类，综合 ORCID、索引 ID、共同 DOI、单位、研究方向、共同作者及专著关系。
- 统一连字符、破折号和姓名倒置写法；ORCID 冲突时继续保持独立，常见同名不得仅凭姓名合并。
- Jay Ke-Schutte 的多个 OpenAlex 档案现在整合为一个结果，并将 2026 年成果作为最新更新。
- 学者主页聚合全部已确认作者 ID 及经过核验的 Crossref 作品；ORCID 仅作为身份锚点，不再被视为完整作品清单。
- 搜索结果显示已整合的索引记录数量和主要证据。

### 本地数据与追踪

- 本地数据升级到 version 5，新增身份核验时间与合并证据。
- 已有重复关注项会在首次成功联网核验后自动合并，保留最早关注日期及全部收藏、已读、忽略和翻译状态。
- 已关注学者每七天最多重新核验一次；新发现的作者 ID 会自动写回本地文件。

### 发布文件

- `Anthropology-Canteen-Windows-v1.1.1.zip`
- SHA-256：`66D6C207AF9B887028A3F52542F948178CAE1349820792C7BB872B0EB7682277`
