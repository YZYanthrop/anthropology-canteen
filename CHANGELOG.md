...
- 统一构建工作流由正常版本标签触发，也可手动重新验证一个已经存在的正常标签；它负责
  在对应原生 runner 构建、测试并上传三个平台的临时构建产物和 SHA-256，但不会创建
  GitHub Release，也不会签名、公证或公开发布。
- 本次只准备源码候选。`v1.2.0` 标签、三平台正式附件和 GitHub Release 尚未创建，
  必须在后续获得明确授权的发布任务中完成。
